...
```bash
violet push local-auth.amd64.v4.2.0.tgz \
...
spec:
  version: v4.2.0                           # plugin version
  config:
    global:
      masterNode: <node-ip-or-host>         # external access address, this address is also used as the external IDP redirect address
      masterDefaultPort: 11780              # external access port
```
...
* `.spec.config`: Plugin configuration. If the corresponding ModuleConfig is empty, this field can be left empty.
* `.spec.config.global.masterNode`: External access address, also used as the external IDP redirect address.
* `.spec.config.global.masterDefaultPort`: External access port (default: 11780).